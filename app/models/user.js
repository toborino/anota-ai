						...
						that.bot.pgClient.query('UPDATE "user_data" SET timezone = $1 WHERE user_id = $2;', [timezone, result.rows[0].user_id], 
							function (err, update_result)
							{
								...
								else
								{
									...
									that.bot.pgClient.query('UPDATE "notes" SET reminder_at = reminder_at + INTERVAL \'$1 minutes\' WHERE user_id = $2 AND timezone IS NULL;', [parseInt(timezone), result.rows[0].user_id]) 
