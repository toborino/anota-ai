						...
						that.bot.pgClient.query('UPDATE "user_data" SET timezone = $1 WHERE user_id = $2;', [timezone, result.rows[0].user_id], 
							function (err, update_result)
							{
								...
								else
								{
									...
									that.bot.pgClient.query('UPDATE "notes" SET reminder_at = reminder_at + INTERVAL \'' + parseInt(timezone) + ' minutes\' WHERE user_id = $1 AND reminder_at IS NOT NULL AND timezone IS NULL;', [result.rows[0].user_id]) 
